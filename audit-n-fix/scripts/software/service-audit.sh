	...
	'systemd')
		...
		for svc_path in "${paths[@]}"; do
			...
			if [[ -h "${svc_path}" ]]; then
				...
				case "${real_path}" in
					/lib/systemd/system/*|/usr/lib/systemd/system/*)
						printf '%s\0' "${svc_path}" "${real_path}" >> "${log_dir}/likely-vendor-services.txt"
						;;
					/dev/null)
						printf '%s\0' "${svc_path}" "${real_path}" >> "${log_dir}/masked-services.txt"
						;;
					*)
			...
			# Directories
				...
				case "${svc_path}" in
					*.d)
						printf '%s\0' "${svc_path}" >> "${log_dir}/service-overrides-dirs.txt"
						;;
					*.wants)
						printf '%s\0' "${svc_path}" >> "${log_dir}/service-dependencies-dirs.txt"
						;;
					*)
