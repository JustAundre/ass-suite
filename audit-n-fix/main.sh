...
cat <<-EOF
	  ...
	  ---{=========}###[@]###{===========}---
	      ...
	      Take the time to get a password
	    manager, and use distinct passwords
		    BEFORE you worry yourself
	         about hardening servers

	  ---{=========}###[@]###{===========}---

	You can find log files for this session here:
	${log_dir}
EOF
exit 0
